...
files_demand <- list.files(path = ".", pattern = "varyingDemand.*\\.csv$", full.names = TRUE)
...
varying_demand_list <- lapply(files_demand, read.csv)

# Schritt 1: XSet extrahieren
varying_demand_list <- lapply(varying_demand_list, function(df) {
...
varying_demand_list <- lapply(varying_demand_list, add_min_columns)
...
make_improvement_plot_id <- function(df) {
  ...
  plot_data <- df %>%
    select(ID, Min_Combined, Min_Euclidean, Min_Road) %>%
    distinct() %>%
    mutate(
  ...
  ggplot(plot_data, aes(x = ID, y = Improvement, color = Method, group = Method)) +
    ...
    labs(
      title = paste("Improvement to Euclidean (var) –", sample_size, "samples"),
      x = "ID",
      y = "Improvement (%)",
      color = "Method"
    ) +
    scale_color_manual(
      values = c("Combined" = "steelblue", "Road" = "springgreen2")
    ) +
...
# Für deine Liste, z. B. varying_demand_list:
plots_id <- lapply(varying_demand_list, make_improvement_plot_id)


for (i in 1:5) {
  ggsave(filename = paste0("plot_varying", i, ".png"), plot = plots_id[[i]], width = 18, height = 8.25, dpi = 300)
}
library(dplyr)
library(ggplot2)
library(patchwork)
...
all_df <- bind_rows(varying_demand_list)
...
ggsave(filename = "plot_var_box1.png", plot = p_combined, width = 18, height = 8.25, dpi = 300)
ggsave(filename = "plot_var_box2.png", plot = p_road, width = 18, height = 8.25, dpi = 300)
